...
export default async function UpdateTask(req, res) {
    ...
    // Grab inputs
    const {taskId,task_title,task_description, status,task_due_date,task_priority,task_category,task_tags} = req.body;
    ...
    const result = await task.UpdateTask(title,description,task_due_date,priority,task_category,task_tags,taskId);
